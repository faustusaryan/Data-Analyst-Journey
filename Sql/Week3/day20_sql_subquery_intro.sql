...
-- Q20  Subquery in WHERE — Single Value
-- Task:- Aise employees dhundho jinki salary company average salary se zyada hai
 
SELECT name, salary
FROM employees
WHERE salary > (SELECT AVG(salary) FROM employees);

-- Q21  Subquery with IN — Multiple Values
-- Task:- Aise employees dhundho jo un departments mein hain jinki location 'Bangalore' ya 'Mumbai' hai. 
-- Subquery use karo (JOIN use mat karo).

SELECT dept_id, name, city
FROM employees
WHERE city IN (SELECT location FROM departments WHERE location IN ('Bangalore', 'Mumbai'));

-- Q22  Subquery in FROM (Derived Table)
-- Task:- Har department mein highest salary wale employee ka naam aur salary dikhao.
...
ON e.dept_id = t.dept_id AND e.salary = t.max_salary
JOIN departments d
