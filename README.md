...
Originally a 60-day plan. After an honest review at Day 21, the plan was restructured.
Not a failure — a smarter approach.
...
| **Power BI**   | Not Started          | ⬜ Upcoming                                      |
| **Statistics** | Not Started          | ⬜ Upcoming                                      |

> Pandas and Excel were "touched" in Phase 1 but not retained. Marked honestly.
...
| Week                | Focus                                          | Status |
| ------------------- | ---------------------------------------------- | ------ |
| Week 1 · Days 1–7   | SQL Fundamentals + Core Python                 | ✅ Done |
| Week 2 · Days 8–14  | SQL Intermediate + Pandas intro + Excel basics | ✅ Done |
| Week 3 · Days 15–21 | Subqueries + Advanced Pandas (struggled here)  | ✅ Done |

### 🔄 Phase 2 — Restructured Plan (Days 22–81)
...
| New Week 4    | Days 43–49   | Power BI + Statistics + Mini Project       | 🔄 Started |
...
| Day 5  | SQL JOINs + 15 JOIN Practice Queries + List Comprehension         |
| Day 6  | HackerRank SQL Problem Solving                                    |
| Day 7  | SQL Practice — DISTINCT, ORDER BY, LIMIT, Subqueries intro        |
| Day 8  | SQL String Functions Practice                                     |
| Day 9  | SQL JOIN + Aggregation + Subqueries                               |
| Day 10 | Python Functions Practice                                         |
| Day 11 | Pandas Intro (Titanic dataset)                                    |
| Day 12 | Excel Basics — VLOOKUP, Pivot                                     |
| Day 13 | SQL BETWEEN Practice                                              |
| Day 14 | JOIN with BETWEEN                                                 |
| Day 15 | SQL CASE WHEN                                                     |
| Day 16 | CASE WHEN with ORDER BY + Python Dict Practice                    |
| Day 17 | Python List of Dicts — Deep Practice                              |
...
| Day 20 | SQL Subqueries (WHERE / IN / Derived Tables) + Core Python        |
...
| Day 37 | **Superstore project started.** Loaded the 9,994-row Superstore dataset into MySQL with `LOAD DATA INFILE` — `order_date` / `ship_date` typed as `DATE` and parsed at load time with `STR_TO_DATE('%m/%d/%Y')`, `postal_code` kept as `VARCHAR` to preserve leading zeros with blanks converted via `NULLIF(TRIM(...))`, script made re-runnable with `IF NOT EXISTS` / `DROP TABLE IF EXISTS`. Post-load validation added: row count, unparsed-date check, date range (2014-01-03 → 2017-12-30), sales/profit totals. 5 business queries — total sales by Category, top 5 customers by sales, sales by Region, monthly sales trend for 2016, top 3 sub-categories per Region (CTE + RANK + PARTITION BY). Q4 had HAVING vs WHERE confusion — corrected. Q5 CTE structure written independently after concept explanation. |
...
<details>
<summary><strong>SQL ✅ — Strongest Area</strong></summary>
...
* Subqueries — Scalar, `IN`/`NOT IN`, `EXISTS`/`NOT EXISTS`, Derived Tables, Correlated, Nested (2nd highest salary, nested FROM in HAVING)
...
* Analytical patterns — running totals, ratio-to-total, month-over-month % change, N-period moving average, quartile segmentation, rank movement year over year, Pareto / cumulative % contribution, per-row contribution % of grand total, discounted vs non-discounted cohort comparison
* CTEs — basic (single), chained (multi-CTE), CTE + Window Function (TOP-N filter), recursive CTE (`WITH RECURSIVE` — hierarchy tree, level tracking, path building) · `IFNULL` · `CROSS JOIN` for scalar CTE values · CTE vs Subquery equivalence (same result, rewritten both ways) · correlated subquery → CTE + JOIN rewrite
...
* Nested dicts, dict comprehension
...
* String methods — `strip()`, `title()`, `lower()`, `replace()`, `split()`, `startswith()`, `endswith()`, `find()`
* Regex — `re.sub()`, `\D`, `[^\d]`
* `type() is` vs `isinstance()` — bool/int subclass edge case
* `all()` / `any()` with generator expressions — short-circuit behavior
...
<details>
<summary><strong>Excel 🔄 · Power BI ⬜ · Statistics ⬜</strong></summary>

* **Excel** — Basic exposure only. Proper coverage in New Week 3.
* **Power BI** — Not started. New Week 4.
* **Statistics** — Not started. New Week 4.
...
> Build a job-ready Data Analyst portfolio for entry-level roles in India.
> Standard: Solve problems independently without copying solutions.

**Focus:** Writing clean, interview-ready SQL with multiple approaches (JOIN vs Subquery vs CTE)

**Next up:** Remaining unaided window-function re-attempt (Day 42 gaps — frames, aggregate-inside-window, stacked windows across CTEs), then the Pandas block carried over from Week 3, then Power BI.
